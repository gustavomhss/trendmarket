...
# carrega .env se existir (mantém portas atuais)
...
# reescreve docker-compose com indentação de 2 espaços
...
# mostra com numeração para auditoria visual
nl -ba docker-compose.observability.yml | sed -n '1,160p'


# valida estrutura
docker compose -f docker-compose.observability.yml config >/dev/null && echo "[compose] ✅ YAML válido" || { echo "[compose] ❌ YAML inválido"; exit 2; }
