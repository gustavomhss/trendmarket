...
**Passos (com fórmulas ASCII e rounding):**
1) Cotação bruta — `dy_out = get_amount_out(R_V, R_STBL, dx, 3_000)` ⇒ **838_295.810577985881513049 STBL** (mesmos roundings de RX1)
...
**Fórmula (ASCII):**
```text
dx_fee = ceil(dx_gross * f_ppm / PPM_SCALE)
dx_net = dx_gross - dx_fee
R_x_prime = R_x + dx_net
R_y_star = round_nearest_even((R_x * R_y) / R_x_prime)
dy_out = R_y - R_y_star
```
...
**Fórmula (ASCII):**
```text
dx_net_est = ceil(R_x * dy_target / (R_y - dy_target))
dx_upper = ceil(dx_net_est * PPM_SCALE / (PPM_SCALE - f_ppm))
dx_gross = argmin_{dx ≥ 1} get_amount_out(R_x, R_y, dx, f_ppm) ≥ dy_target
```
...
**Fórmula (ASCII):**
```text
p_spot_xy = round_nearest_even((R_y * WAD) / R_x)
```
...
**Notas:** Usa a rotina ensure_reserves e escala o quociente para WAD antes do arredondamento `nearest-even`.

### Spot price Y em X
**Fórmula (ASCII):**
```text
p_spot_yx = round_nearest_even((R_x * WAD) / R_y)
```
...
**Fórmula (ASCII):**
```text
dy_out = get_amount_out(R_x, R_y, dx_gross, f_ppm)
p_exec_xy = round_nearest_even((dy_out * WAD) / dx_gross)
```
...
**Notas:** Propaga os guardrails da rotina get_amount_out e converte o resultado bruto em preço efetivo na mesma escala WAD.

### Slippage relativo X→Y
**Fórmula (ASCII):**
```text
p_spot_xy = spot_price_x_in_y(R_x, R_y)
p_exec_xy = execution_price_x_to_y(R_x, R_y, dx_gross, f_ppm)
raw_ppm = round_nearest_even(((p_spot_xy - p_exec_xy) * PPM_SCALE) / p_spot_xy)
slip_ppm = min(raw_ppm, PPM_SCALE)
```
...
**Fórmula (ASCII):**
```text
tol_eff = min(tol_ppm, PPM_SCALE)
dy_out = get_amount_out(R_x, R_y, dx_gross, f_ppm)
dy_min = floor(dy_out * (PPM_SCALE - tol_eff) / PPM_SCALE)
```
...
```text
tol_eff = min(tol_ppm, PPM_SCALE)
dx_core = get_amount_in(R_x, R_y, dy_target, f_ppm)
dx_max = ceil(dx_core * (PPM_SCALE + tol_eff) / PPM_SCALE)
```
...
**Fórmula (ASCII):**
```text
shares_mint = floor(sqrt(R_x * R_y))
```
...
**Fórmula (ASCII):**
```text
ratio_x = dx_add * S_tot / R_x
ratio_y = dy_add * S_tot / R_y
shares_mint = floor(min(ratio_x, ratio_y))
```
...
**Fórmula (ASCII):**
```text
x_withdraw = floor(R_x * S_burn / S_tot)
y_withdraw = floor(R_y * S_burn / S_tot)
```
...
As decisões e o inventário de CI estão versionados junto ao código:
