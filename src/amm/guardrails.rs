...
pub fn checked_add(a: Wad, b: Wad) -> Result<Wad, AmmError> {
    a.checked_add(b).ok_or(crate::amm_err!(
        crate::amm::error_catalog::AmmErrorCode::OverflowNumericNumeric
    ))
}
...
pub fn u256_to_u128_checked(v: U256) -> Result<Wad, AmmError> {
    if v > U256::from(u128::MAX) {
        crate::amm_bail!(crate::amm::error_catalog::AmmErrorCode::OverflowNumericNumeric)
    } else {
        Ok(v.as_u128())
    }
}
...
mod tests {
    ...
    fn t_ensure_reserves() {
        ...
        // zero
        assert_eq!(
            ensure_reserves(0, MIN_RESERVE).unwrap_err().code,
            crate::amm::error_catalog::AmmErrorCode::ZeroReserve
        );
        assert_eq!(
            ensure_reserves(MIN_RESERVE, 0).unwrap_err().code,
            crate::amm::error_catalog::AmmErrorCode::ZeroReserve
        );
        // abaixo do mínimo
        assert_eq!(
            ensure_reserves(MIN_RESERVE - 1, MIN_RESERVE)
                .unwrap_err()
                .code,
            crate::amm::error_catalog::AmmErrorCode::MinReserveBreached
        );
        assert_eq!(
            ensure_reserves(MIN_RESERVE, MIN_RESERVE - 1)
                .unwrap_err()
                .code,
            crate::amm::error_catalog::AmmErrorCode::MinReserveBreached
        );
    ...
    fn t_checked_add_sub_over_under_flow() {
        ...
        // add overflow
        assert_eq!(
            checked_add(UMAX, 1).unwrap_err().code,
