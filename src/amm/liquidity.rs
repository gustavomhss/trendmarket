...
use super::guardrails::{
    checked_add, checked_sub, div_nearest_even_u256_to_u128, mul_u128_to_u256, u256_to_u128_checked,
...
mod tests {
    ...
    fn t_add_liquidity_proportional_sym() {
        let (x, y, s) = (1_000_000u128, 1_000_000u128, 1_000_000u128);
        let (dx, dy) = (100_000u128, 100_000u128);
        let mint = add_liquidity(x, y, dx, dy, s).unwrap();
        assert_eq!(mint, 100_000);
    }

    #[test]
    fn t_add_liquidity_min_by_y() {
        let (x, y, s) = (1_000_000u128, 1_000_000u128, 1_000_000u128);
        let (dx, dy) = (200_000u128, 100_000u128); // y limita
        let mint = add_liquidity(x, y, dx, dy, s).unwrap();
        assert_eq!(mint, 100_000);
    }
    ...
    fn t_add_liquidity_too_small() {
        let (x, y, s) = (1_000_000u128, 1_000_000u128, 1_000_000u128);
        let (dx, dy) = (1u128, 1u128); // muito pouco → zero shares
    ...
    fn t_remove_liquidity_10_percent() {
        let (x, y, s) = (1_000_000u128, 1_000_000u128, 1_000_000u128);
        let burn = 100_000u128; // 10%
        let (xo, yo) = remove_liquidity(x, y, burn, s).unwrap();
        assert_eq!((xo, yo), (100_000, 100_000));
        // remanescentes >= min
        let (x1, y1) = (x - xo, y - yo);
    ...
    fn t_remove_liquidity_burn_too_big() {
        let (x, y, s) = (2_000_000u128, 2_000_000u128, 1_000_000u128);
    ...
    fn t_remove_liquidity_zero_burn() {
        let err = remove_liquidity(1_000_000, 1_000_000, 0, 1_000_000).unwrap_err();
        assert_eq!(err, AmmError::ZeroAmount);
    }

    #[test]
    fn t_remove_liquidity_min_reserve_guard() {
        // reservas pequenas: queimar muito viola mínimo
        let (x, y, s) = (MIN_RESERVE + 10, MIN_RESERVE + 10, 1_000_000u128);
        // escolha burn proporcional pra tentar levar abaixo do mínimo
        let err = remove_liquidity(x, y, 999_999, s).unwrap_err();
        assert_eq!(err, AmmError::MinReserveBreached);
    }
}
